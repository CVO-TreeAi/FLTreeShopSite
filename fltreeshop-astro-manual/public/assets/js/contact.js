...
jQuery(document).ready(function() {
	...
	jQuery('#ContactThanksMsg').hide();

	jQuery('#ContactBtnForm').click(function(e) {

		e.preventDefault();

		jQuery("#FormContact").trigger('submit') ;
...
function scrollpages($Id) {

	$Top = jQuery('#'+$Id).offset().top ;

	jQuery('html, body').animate({ scrollTop: $Top -200 }, 1000);
...
function submitFormContact() {
		...
		ajaxobj = jQuery.ajax({
					...
					url: "thanks-contact.php", //"../thanks-contact.php"
					...
					beforeSend: function() {
						...
						jQuery('#HideContactFormX').hide();

						jQuery('#LoaderContact').show();
					...
					success: function(html){
						...
						jQuery('#ContactThanksMsg').show();

						scrollpages('ContactThanksMsg');
